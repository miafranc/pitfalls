'''
From: https://github.com/M-Nauta/PIPNet/blob/main/util/preprocess_cub.py
...
for k in range(num):
    ...
    if int(split[k][0][-1]) == 1:
        ...
        if not os.path.isdir(os.path.join(train_save_path, file_name)):
            os.makedirs(os.path.join(train_save_path, file_name))
        img = Image.open(os.path.join(path, 'images', images[k][0].split(' ')[1])).convert('RGB')
        x, y, w, h = bboxes[id]
        cropped_img = img.crop((x, y, x+w, y+h))
        cropped_img.save(os.path.join(train_save_path, file_name, images[k][0].split(' ')[1].split('/')[1]))
        ...
        x, y, w, h = bboxes[id]
        cropped_img = img.crop((x, y, x+w, y+h))
        cropped_img.save(os.path.join(test_save_path, file_name, images[k][0].split(' ')[1].split('/')[1]))
        print('%s' % images[k][0].split(' ')[1].split('/')[1])
...
for k in range(num):
    ...
    if int(split[k][0][-1]) == 1:
        ...
        img.save(os.path.join(train_save_path, file_name ,images[k][0].split(' ')[1].split('/')[1]))
        
        print('%s' % images[k][0].split(' ')[1].split('/')[1])
    else:
        if not os.path.isdir(os.path.join(test_save_path, file_name)):
            os.makedirs(os.path.join(test_save_path, file_name))
        shutil.copy(os.path.join(path, 'images', images[k][0].split(' ')[1]), os.path.join(test_save_path, file_name, images[k][0].split(' ')[1].split('/')[1]))
        print('%s' % images[k][0].split(' ')[1].split('/')[1])
time_end = time.time()
